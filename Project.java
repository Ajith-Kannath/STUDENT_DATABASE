    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        try
        {
          ...
          if(s.equals("SIGN IN"))
          {
            ...
            if(b)
            {
              r = st.executeQuery("SELECT NAME, CLASS , ADMN, TO_CHAR(DOB, 'DD Mon YYYY'), ADDRESS FROM PROJECT WHERE ( NAME ='"+user+"' AND ADMN ='"+pass+"')");
              while(r.next())
              {
                name = r.getString(1);
                clas = r.getString(2);
          ...
          if(s.equals("VIEW DATA"))
          {
            ...
            else
            {
              ...
              if(b)
              {
                r = st.executeQuery("SELECT NAME, CLASS , ADMN, TO_CHAR(DOB, 'DD Mon YYYY'), ADDRESS FROM PROJECT WHERE ADMN ='"+admn+"'");
                while(r.next())
                {
                  name = r.getString(1);
                  clas = r.getString(2);
          ...
          {
            r = st.executeQuery("SELECT NAME, CLASS , ADMN, TO_CHAR(DOB, 'DD Mon YYYY'), ADDRESS FROM PROJECT ORDER BY ADMN");
            while(r.next())
            {
              name = r.getString(1);
              clas = r.getString(2);
